...
unsigned long align_4(unsigned long align){
	unsigned long check = align & 3;
	align = (align >>2)<<2;
	if(check) align  = align + 4;
	return align;
}
void eFPartitionWalkByAddr(unsigned long efpHandler){
	...
	*/
	...
	EEB * eeb =(EEB *) handler->firstFree;
	while((unsigned long)eeb){//假定最后一个eeb代指的next是0
		showEEB(eeb);
...
unsigned long eFPartitionTotalSize(unsigned long perSize, unsigned long n){
	...
	EEB不放在persize中,也就是EEB独立的占用部分的内存
	*/
	perSize = align_4(perSize);
	return perSize*n +sizeof(eFPartition);
...
unsigned long eFPartitionInit(unsigned long start, unsigned long perSize, unsigned long n){
	...
	*/
	...
	perSize = align_4(perSize);
	handler->perSize = perSize;//相关的对齐的内容
	handler->totalN = n;
	handler->firstFree = start + sizeof(eFPartition);
	unsigned long addr;
	addr = handler->firstFree;
	int i;
	EEB *eeb;
	for(i=0;i<n;i++){//完成n块的初始化.
	eeb = (EEB *)addr;
	eeb->next_start=addr+perSize;
	addr = eeb->next_start;
	}
	eeb->next_start = 0;//最后一块无后,标记为0
...
unsigned long eFPartitionAlloc(unsigned long EFPHandler){
	...
	*/
	eFPartition * handler  = (eFPartition *)EFPHandler;
	if(handler->firstFree == 0) return 0;//表示分配失败,提前结束
	EEB * eeb = (EEB *)handler->firstFree;
	handler->firstFree = eeb->next_start;
	return (unsigned long)eeb;//EEB独自占用内存	
	//以上直接把EEB分配出去,这里的数据结构不需要通过EEB中的数据结构进行维护
}
...
unsigned long eFPartitionFree(unsigned long EFPHandler,unsigned long mbStart){
	...
	*/
	...
	unsigned long addr = handler->firstFree;//存放第一个空闲EEB块的地址
	EEB *eeb;
	eeb = (EEB *)mbStart;
	eeb->next_start = handler->firstFree;
